...
/**
 * Preprocess HTML to preserve colons in Alpine.js attributes
 * The browser's DOMParser removes colons from attribute names (per HTML spec)
 * We encode them as __colon__ before parsing, then decode them back
 * NOTE: Using lowercase __colon__ because DOMParser lowercases all attribute names
 * @param {string} html - HTML string
 * @returns {string} Processed HTML with encoded colons
 */
...
function getAttributes(element) {
  ...
  Array.from(element.attributes).forEach(attr => {
    ...
    // Extract block name from data-block-name or id (don't add to globalAttrs)
      ...
      attrName === 'data-block-name' ||
      attrName === 'data-block-id' ||
      attrName === 'id'
    ) {
