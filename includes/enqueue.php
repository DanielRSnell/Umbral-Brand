...
function blank_theme_enqueue_block_assets() {
    ...
    // Handle CSS files if enabled
    if (!empty($settings['src']['styles']) && is_dir($production_dir)) {
        $css_files = glob($production_dir . '/*.css');

        // Check if inline_css is enabled
        if (!empty($settings['inline_css'])) {
            // Inline CSS directly in the head
            foreach ($css_files as $css_file) {
                $css_content = file_get_contents($css_file);
                if ($css_content !== false) {
                    echo '<style id="blank-theme-' . pathinfo($css_file, PATHINFO_FILENAME) . '-inline-css">' . $css_content . '</style>';
                }
            }
        } else {
