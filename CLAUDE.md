...
This theme uses a comprehensive **glass morphism design system** built on Tailwind CSS v4. **ALWAYS** reference the official design guide before making styling decisions.

### Official Documentation

📖 **[src/docs/styles-guide.md](src/docs/styles-guide.md)** - Complete Design System Documentation

This guide contains:
- **Color System**: Full neutral palette, accent colors, glass morphism colors
- **Glass Morphism Components**: `.glass-card`, `.glass-section`, `.glass-badge`, `.glass-stat`, `.glass-button`, `.glass-panel`
- **Typography System**: Hero scale, heading hierarchy, body text patterns
- **Spacing & Layout**: Section patterns, grid systems, responsive breakpoints
- **Component Patterns**: Buttons, cards, stats, badges, timelines, accordions
- **Visual Effects**: Borders, shadows, animations, image treatments
- **Quick Reference Tables**: Most-used utilities, component selectors

### Core Design Principles

When creating or modifying components, follow these principles:

1. **Glass Morphism First**: Default to glass components (`.glass-*`) for containers
2. **Neutral Palette**: Use `neutral-*` colors (200 for borders, 600 for body, 900 for headings)
3. **Consistent Typography**: Almost always use `tracking-tight` on text
4. **Generous Rounding**: `rounded-3xl` for sections, `rounded-full` for buttons/badges
5. **Minimal Shadows**: Subtle, barely-there depth effects
6. **Smooth Transitions**: 200-300ms ease transitions
7. **Icon Standards**: `w-4 h-4` with `stroke-width="1.5"`

### Glass Component Quick Reference

| Component | Use Case |
|-----------|----------|
| `.glass-card` | Article cards, featured content blocks |
| `.glass-section` | Large section backgrounds, container panels |
| `.glass-badge` | Tech tags, labels, pills |
| `.glass-stat` | Metrics, numbers, stats displays |
| `.glass-button` | Secondary/ghost buttons, icon buttons |
| `.glass-panel` | Accordions, expandable content |

### Common Patterns

**Container Structure**:
```html
<section class="max-w-7xl mx-auto px-4 sm:px-6 mt-16 sm:mt-20">
...
<a href="#" class="inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-medium tracking-tight text-neutral-50 bg-neutral-900 hover:bg-neutral-800 border border-neutral-900">
  <svg class="w-4 h-4" stroke-width="1.5">...</svg>
...
<article class="glass-card overflow-hidden">
  <div class="relative aspect-[16/9]">
    <img src="..." alt="..." class="w-full h-full object-cover grayscale saturate-[0.7] contrast-[1.1]" />
  </div>
...
**Headings**:
```html
<h1 class="text-3xl sm:text-5xl font-bold tracking-tight text-neutral-900">
...
**Section Spacing**:
```css
mt-16 sm:mt-20           /* Between sections */
...
1. ✅ Check [styles-guide.md](src/docs/styles-guide.md) for existing patterns
2. ✅ Use glass morphism components when appropriate
3. ✅ Follow established color hierarchy (neutral-900 → neutral-600 → neutral-500)
4. ✅ Match spacing patterns (standard gaps, padding values)
5. ✅ Maintain typography consistency (tracking-tight, font-semibold, etc.)
6. ✅ Keep responsive breakpoint patterns consistent
7. ✅ Use standard icon sizing (`w-4 h-4`) and stroke width (`1.5`)

### Don't Break the System

❌ **Avoid**:
- Custom colors outside neutral palette (unless semantic/accent)
- Thick borders (stick with 1px `border-neutral-200`)
- Text without `tracking-tight`
- Sharp corners on major elements (use `rounded-3xl` or `rounded-full`)
...
- Keep transitions uniform (200-300ms ease)
...
#### Alpine.js Attribute Support (Fixed 2025-01-26)
The WordPress block parser now correctly preserves colons in Alpine.js attributes. Both full syntax and shorthand work:

**Supported Alpine.js Attributes:**
```html
<!-- Full syntax (recommended for WordPress) -->
...
**How it works:**
The parser preprocesses HTML to encode colons as `__colon__` before the browser's DOMParser processes attributes, then decodes them back to `:` when generating block markup. This preserves Alpine.js syntax while working within HTML spec constraints.

**Implementation:** See `includes/client-html2blocks.js` functions:
- `preprocessAlpineAttributes()` - Encodes colons before parsing
- `getAttributes()` - Decodes colons after parsing
